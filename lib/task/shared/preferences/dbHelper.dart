...
class DbHelper {
  ...
  Future<List<String>> getAllTasksNames() async {
    final db = _db!.database;
    final List<Map<String, dynamic>> maps = await db.rawQuery('SELECT taskName FROM tasks;');
    return List.generate(maps.length, (index) => maps[index]['taskName'].toString());
  }
  ...
  Future<NestedTaskModel> showNestedTask(int id) async {
    ...
    final maps = await db.query('nestedTasks', where: 'id = ?', whereArgs: [id]);
  ...
  Future<List<NestedTaskModel>> showAllNestedTasks(
      int mainTaskId) async {
    final db = _db!.database;
  ...
  Future<int> updateNestedTask(NestedTaskModel nestedTask, int id) async {
    ...
    return db
        .update('nestedTasks', nestedTask.toMap(), where: 'id = ?', whereArgs: [id]);
  }
  ...
  Future<TaskDaysModel> showTaskDays(int mainTaskId) async {
    ...
    final maps = await db.query('taskDays', where: 'mainTaskId = ?', whereArgs: [mainTaskId]);
  ...
  Future<int> deleteTaskDays(int mainTaskId) async {
    ...
    return db.delete('taskDays', where: 'mainTaskId = ?', whereArgs: [mainTaskId]);
  }
